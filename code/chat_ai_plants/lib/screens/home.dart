...
Widget build(BuildContext context) {
  return Scaffold(
    appBar: AppBar(
      title: const Text('World of plants'),
      actions: [
        IconButton(
            onPressed: () {
              showDialog(
    ...
    body: ValueListenableBuilder(
      valueListenable: Hive.box<ChatItem>('chats').listenable(),
      builder: (context, Box<ChatItem> box, _) {
        if (box.isEmpty) return const Center(child: Text('No chats yet'));
        return ListView.builder(
          itemCount: box.length,
          itemBuilder: (context, index) {
            final chatItem = box.getAt(index);
            return ListTile(
    ...
    floatingActionButton: FloatingActionButton.extended(
      onPressed: () {
        ...
        // add to hive
           Hive.box<ChatItem>('chats').add(chatItem);
